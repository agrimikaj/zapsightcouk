...
import { Target, Copy, RefreshCw } from 'lucide-react';

interface AIVIConvergenceDiagramProps {
  onNodeHover?: (nodeId: string | null) => void;
  highlightedNode?: string | null;
}

const AIVIConvergenceDiagram = ({ onNodeHover, highlightedNode }: AIVIConvergenceDiagramProps) => {
  ...
  const nodes = [
    { id: 'intent', label: 'Intent', caption: 'Scenarios, not strings', icon: Target, angle: -90 },
    { id: 'fidelity', label: 'Fidelity', caption: 'Perception aligned with essence', icon: Copy, angle: 150 },
    { id: 'resilience', label: 'Resilience', caption: 'Persistent through evolution', icon: RefreshCw, angle: 30 },
  ];
    ...
    <div className="relative w-full max-w-xl mx-auto aspect-square">
      ...
      <svg viewBox="0 0 400 400" className="w-full h-full">
        ...
        <motion.circle
          ...
          r="140"
          fill="none"
          stroke="url(#aiviRotatingGradient)"
          strokeWidth="1.5"
        ...
        <defs>
          <linearGradient id="aiviRotatingGradient" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="hsl(var(--primary))" stopOpacity="0.9" />
            <stop offset="50%" stopColor="hsl(var(--primary))" stopOpacity="0.15" />
            <stop offset="100%" stopColor="hsl(var(--primary))" stopOpacity="0.9" />
          </linearGradient>
          <radialGradient id="aiviCoreGlowGradient" cx="50%" cy="50%" r="50%">
      ...
      >
        <span className="font-display text-lg font-bold text-primary">AIVI</span>
        <span className="text-[hsl(220,10%,50%)] text-[10px] uppercase tracking-widest mt-1">Unified Brand</span>
        <span className="text-[hsl(220,10%,50%)] text-[10px] uppercase tracking-widest">Truth</span>
      </motion.div>
          ...
          >
            ...
            >
              ...
              {isActive && (
                ...
                >
                  <span className="text-[hsl(220,10%,70%)] text-xs whitespace-nowrap italic">
                    {node.caption}
                  </span>
                </motion.div>
              )}
      ...
      {/* AI Platform icons at outer ring */}
      <div className="absolute top-2 left-1/2 -translate-x-1/2 flex gap-3 opacity-40">
        <div className="w-5 h-5 border border-[hsl(220,10%,25%)] rounded flex items-center justify-center text-[8px] text-[hsl(220,10%,40%)]">
          GPT
        </div>
        <div className="w-5 h-5 border border-[hsl(220,10%,25%)] rounded flex items-center justify-center text-[8px] text-[hsl(220,10%,40%)]">
          G
...
export default AIVIConvergenceDiagram;
